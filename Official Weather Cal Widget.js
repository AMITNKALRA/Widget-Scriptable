...
/* if (yesterdayTasks.length > 0) {
  ...
  // const heading = widget.addText("Reminders:");
    yesterdayTasks.sort(sortItems).slice(0, 2).forEach(({ title, dueDate }) => {

        if (dueDate < currentDate) {

      const task = widget.addText(`❕❕ ${title}`);
      task.textColor = Color.white()
      task.font = Font.regularSystemFont(16)
      task.lineLimit = 1;
...
if (todayTasks.length == 0 && weatherSettings.tomorrowShownAtHour >= 19 && tomorrowTasks.length > 0) { 
...
async function setupEvents() {
  ...
  let multipleTomorrowEvents = false
    ...
    if (weatherSettings.tomorrowShownAtHour == 19) {
...
async function setupGradient() {
  ...
  let gradient = {
    dawn: {
      color() { return [new Color("327ef6"), new Color("1c4475"), new Color("7f89fb")] },
      position() { return [0, 0.5, 1] },
    },

    sunrise: {
      color() { return [new Color("ffcc2a"), new Color("ff9fa6"), new Color("f0b35e")] },
      position() { return [0, 0.8, 1.5] },
    },

    midday: {
      color() { return [new Color("83cfff"), new Color("3fb3ff")] },
      position() { return [0, 1] },
    },
    ...
    sunset: {
      color() { return [new Color("e7aa33"), new Color("fbd577"), new Color("ff980b")] },
      position() { return [0.1, 0.9, 1.2] },
    },

    twilight: {
      color() { return [new Color("021033"), new Color("16296b"), new Color("151b69")] },
      position() { return [0, 0.5, 1] },
    },
